...
const debug = require('debug')(`cnn-hapi:${path.basename(__filename)}`);
...
class Config {
  constructor({description, name, version}, options, basePath) {
    const internals = {
      defaults: {
        ...
        surrogateCacheControl:
          process.env.SURROGATE_CACHE_CONTROL ||
          options.surrogateCacheControl ||
    ...
    if (process.env.SHOW_CNN_HAPI_CONFIG != undefined && process.env.SHOW_CNN_HAPI_CONFIG === 'true') {
      debug('Server configurations %o', internals.defaults);
    }
    ...
    internals.schema = {
      ...
      host: Joi.string()
        ...
        .example('0.0.0.0'),
      loaderIoValidationKey: Joi.string().description('loader.io validation'),
    ...
    this.settings = Object.assign(internals.defaults, {description, name, version});
    ...
    this.settings.isProduction = () => {
      ...
      return environment.toLowerCase() === 'production' || environment.toLowerCase() === 'prod';
    };
  }

  get httpConnection() {
    const {host, labels, port} = this.settings;
  ...
  get httpsConnection() {
    const {cert, key, port = 3000} = this.settings.localTLS;

    const {basePath, host, labels} = this.settings;
